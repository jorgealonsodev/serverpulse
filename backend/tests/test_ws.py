...
async def test_ws_connect_valid_token(client: AsyncClient, ws_client_factory):
...
async def test_ws_initial_status_on_connect(client: AsyncClient, ws_client_factory):
...
async def test_ws_receive_metric_on_ingest(client: AsyncClient, ws_client_factory):
...
async def test_ws_status_change_offline(client: AsyncClient, ws_client_factory):
...
async def test_ws_multiple_connections(client: AsyncClient, ws_client_factory):
    ...
    async with (  # noqa: SIM117
        ...
        async with aconnect_ws(f"/api/v1/ws?token={token}", ws1_client) as ws1:
            async with aconnect_ws(f"/api/v1/ws?token={token}", ws2_client) as ws2:
                    ...
                    # (both should in production, but test isolation can cause race conditions)
